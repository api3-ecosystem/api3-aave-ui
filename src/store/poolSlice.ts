...
export interface PoolSlice {
  ...
  debtSwitch: (args: any) => PopulatedTransaction;
  setUserEMode: (
    categoryId: number,
  ) => Promise<EthereumTransactionTypeExtended[]>;
  signERC20Approval: (args: Omit<any, "user">) => Promise<string>;
  claimRewards: (args: any) => Promise<EthereumTransactionTypeExtended[]>;
  // TODO: optimize types to use only neccessary properties
  swapCollateral: (args: any) => Promise<EthereumTransactionTypeExtended[]>;
  repay: (args: any) => Promise<EthereumTransactionTypeExtended[]>;
  repayWithPermit: (
    args: any & {
  ...
  supply: (args: Omit<any, "user">) => PopulatedTransaction;
  supplyWithPermit: (args: Omit<any, "user">) => PopulatedTransaction;
  getApprovedAmount: (args: { token: string }) => Promise<ApproveType>;
  borrow: (args: Omit<any, "user">) => PopulatedTransaction;
...
> = (set, get) => {
  ...
  return {
    ...
    supplyWithPermit: (args: Omit<any, "user">) => {
    ...
    setIsFaucetPermissioned: (value: boolean) =>
      set({ isFaucetPermissioned: value }),
    mint: async (args) => {
      const {
        jsonRpcProvider,
        currentMarketData,
    ...
    estimateGasLimit: async (tx: PopulatedTransaction) => {
      ...
      tx.gasLimit = estimatedGas.gt(defaultGasLimit)
        ? estimatedGas
        : defaultGasLimit;
      console.log("borrow trx with gasLimit", tx);
      return tx;
    },
  };
};
