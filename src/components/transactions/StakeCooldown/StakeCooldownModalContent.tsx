...
export const StakeCooldownModalContent = ({
  ...
  const stakedAmount =
    stakeUserResult?.[stakeAssetName as StakingType].stakeTokenRedeemableAmount;
  ...
  if (txState.success)
    return <TxSuccessView action={<div>Stake cooldown activated</div>} />;
  ...
  const amountToCooldown = formatEther(
    userStakeData?.stakeTokenRedeemableAmount || 0,
  );
  return (
    <>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Box>
            ...
            <Typography variant="subheader2" color="error.main">
              <div>{timeMessage(stakeCooldownSeconds)}</div>
            </Typography>
          </Box>
          <Box sx={{ textAlign: "right" }}>
            <Typography variant="helperText" mb={1}>
              <div>Unstake window</div>
            </Typography>
            <Typography variant="subheader2" color="success.main">
              <div>{timeMessage(stakeUnstakeWindow)}</div>
            </Typography>
          </Box>
        </Box>
      </Box>
